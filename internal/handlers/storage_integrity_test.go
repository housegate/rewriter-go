...
func TestStorageIntegrityCallableInFunctionsFailClosed(t *testing.T) {
	...
	for _, sql := range []string{
		...
		`CREATE TABLE other.x AS SELECT inIgnoreSet(id, hg_safe.db1__t) FROM other.u`,
		`INSERT INTO other.u SELECT notNullInIgnoreSet(id, hg_unsafe.db1__t) FROM other.v`,
		`CREATE VIEW other.v AS SELECT globalNullInIgnoreSet(id, hg_safe.db1__t) FROM other.u`,
	} {
		t.Run(sql, func(t *testing.T) {
			ast, err := e.ParseOne(sql)
			if err != nil {
