...
- Storage-integrity (Spec G) goldens live in `internal/harness/testdata/storage_integrity_cases.json`; the C++ repo carries a byte-identical copy — change both in lockstep.
- Storage-integrity lookup precedence selects a physical surface but does not authorize it: logical SI databases must be present in `database_map`, and configured safe/unsafe physical names must reject direct addressing with an SI-flagged accessed table.
